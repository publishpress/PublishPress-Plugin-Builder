...
class PluginVersionHandlerCest
{
     ...
     * @example {"version": "3.4.2"}
     * @example {"version": "3.14.2"}
     * @example {"version": "3.14.52"}
     * @example {"version": "13.14.5"}
     */
    public function setPluginVersion_ForStableVersion_ShouldUpdateTheStableTagVersionInTheReadmeFile(
        UnitTester $I,
        \Codeception\Example $example
    ) {
        $tmpDirPath = sys_get_temp_dir() . '/' . microtime(true);
        ...
        $handler = \Codeception\Stub::make(
            '\\PublishPressBuilder\\PluginVersionHandler',
            [
                'isStableVersion' => true,
            ],
        );

        $handler->setPluginVersion($tmpDirPath, 'publishpress-dummy', $example['version']);

        $tmpFileContent = file_get_contents($tmpDirPath . '/readme.txt');

        $I->assertStringNotContainsString('Stable tag: 2.4.0', $tmpFileContent);
        $I->assertStringContainsString('Stable tag: ' . $example['version'], $tmpFileContent);
    }
     ...
     * @throws Exception
     *
     * @example {"version": "3.4.0-alpha.1"}
     * @example {"version": "3.4.0-beta.1"}
     * @example {"version": "3.54.0-beta.1"}
     * @example {"version": "3.54.0-rc.1"}
     * @example {"version": "3.54.0-rc.2"}
     */
    public function setPluginVersion_ForUnstableVersion_ShouldUpdateTheStableTagVersionInTheReadmeFile(
        UnitTester $I,
        \Codeception\Example $example
    ) {
        $tmpDirPath = sys_get_temp_dir() . '/' . microtime(true);
        ...
        $I->assertStringContainsString('Stable tag: 2.4.0', $tmpFileContent);
        $I->assertStringNotContainsString('Stable tag: ' . $example['version'], $tmpFileContent);
    }
}
